...
public class SubscriptionInitializer {
    ...
    private void subscribe(final String tenantId) {
        contextService.runForTenant(tenantId, () -> {
            ...
            final Optional<MqttSubscriber> subscriberOptional = client.getSubscriber(subscriberId);
            if (subscriberOptional.isEmpty()) {
                log.info("Subscribing to {}", subscriberId);
                final MqttSubscriber subscriber = client.buildSubscriber(subscriberConfig()
                        .id(subscriberId)
            ...
            log.info("MQTT Service client subscribed for tenant {} under {} topic", tenantId, topicProvider.getTopicName());
        });
    }

    @EventListener
    public void subscriptionRemoved(MicroserviceSubscriptionRemovedEvent event) {
        client.closeSubscriber(getSubscriberId(event.getTenant(), topicProvider.getTopicName()));
    }
