...
public class QueclinkPowerTest {
    ...
    public void shouldSendPowerOffAlarmOnGV75Report() {
        String powerOff = "+RESP:GTPFA,3C0101,135790246811220,,20090214093254,11F0$";
        
        queclinkPower.onParsed((new ReportContext(connectionDetails, 
                powerOff.split(QUECLINK.getFieldSeparator()))));
        
        verify(device).powerAlarm(true, false);
        ...
        verify(device).powerAlarm(true, true);
    }
    
    @Test
    public void shouldSendPowerOnAlarmOnGV75Report() {
        String powerOn = "+RESP:GTPNA,3C0101,135790246811220,,20090214093254,11F0$";
        
        queclinkPower.onParsed((new ReportContext(connectionDetails, 
                powerOn.split(QUECLINK.getFieldSeparator()))));
        
        verify(device).powerAlarm(false, false);
        ...
        verify(device).powerAlarm(false, true);
    }
