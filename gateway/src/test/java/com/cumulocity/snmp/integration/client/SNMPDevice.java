package com.cumulocity.snmp.integration.client;
...
public class SNMPDevice extends BaseAgent {
    ...
    private SNMPDevice(String address, String engineId) {

        /*These files does not exist and are not used but has to be specified
        Read snmp4j docs for more info*/

        /*EngineId 123456 and engineId which is received from Integration tests should match.
        Else, Simulator won't return value*/
        super(new File("conf.agent"), new File("bootCounter.agent"),
                new CommandProcessor(new OctetString(engineId)));
    ...
    public static void stopSnmpSimulator() {
        log.debug("SNMP Device Shutting down!");
        device.stop();
    }
    ...
    protected final void addUsmUser(final USM usm) {
        ...
        user = new UsmUser(new OctetString("adminUser"),
                AuthMD5.ID,
                new OctetString("MD5AuthPassword"),
                PrivDES.ID,
                new OctetString("DESPrivPassword"));
        usm.addUser(user.getSecurityName(), usm.getLocalEngineID(), user);

        user = new UsmUser(new OctetString("testUser"),
                AuthSHA.ID,
                new OctetString("testAuthPass"),
                PrivDES.ID,
