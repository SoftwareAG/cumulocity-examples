...
public class DeviceInterface implements CommandResponder {
    ...
    private void addOrUpdateSnmpV3Credentials(Device device) {
        ...
        if (engineID == null || engineID.length() < MPv3.MINLEN_ENGINE_ID || engineID.length() > MPv3.MAXLEN_ENGINE_ID) {
            String errorMsgEngineId = "User '" + device.getUsername() +
                    "' is not added because of an engine ID of incorrect length. The length should be greater than 4 and less than 33.";
            log.error(errorMsgEngineId);
            return;
        }
